...
function checkDB() {
	...
	if($count != 2) {
		$query = "CREATE TABLE IF NOT EXISTS `pic_pictures` (`pic_id` INTEGER, `pic_path`	TEXT NOT NULL, `pic_type` TEXT NOT NULL, `pic_taken` INTEGER NOT NULL, `pic_EXIF` TEXT, `user_id` INTEGER NOT NULL, PRIMARY KEY(`pic_id` AUTOINCREMENT), UNIQUE(`pic_path`,`user_id`), FOREIGN KEY(`user_id`) REFERENCES `users`(`user_id`) ON DELETE CASCADE)";
		$dbh->query($query);
		$query = "CREATE TABLE IF NOT EXISTS `pic_shares` (`share_id` INTEGER,`share_name` TEXT NOT NULL,`share_link` TEXT NOT NULL, `expire_date` INTEGER, `user_id` INTEGER NOT NULL, PRIMARY KEY(`share_id` AUTOINCREMENT), FOREIGN KEY(`user_id`) REFERENCES `users`(`user_id`) ON DELETE CASCADE)";
		$dbh->query($query);
